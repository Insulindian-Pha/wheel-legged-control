...
controller_interface::CallbackReturn VMCController::on_configure(const rclcpp_lifecycle::State& /*previous_state*/)
{
  ...
  anti_split_pid_ = std::make_shared<PID::PidROS>(get_node(), "pid_gains.anti_split", true, true);
  ...
  RCLCPP_INFO(get_node()->get_logger(), "Anti-split compensation: %s", enable_anti_split_ ? "enabled" : "disabled");
...
controller_interface::return_type VMCController::update(const rclcpp::Time& time, const rclcpp::Duration& period)
{
  ...
  {
    double theta_sum = left_current_theta + right_current_theta;  // 目标为 0，即期望两腿 theta 之和为 0
    double leg_tp = anti_split_pid_->update(static_cast<float>(theta_sum), time);
    left_Tp_ += leg_tp;
    right_Tp_ += leg_tp;
  }
