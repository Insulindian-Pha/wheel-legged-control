...
void VMCLeg::calc1Right(double pitch, double pitch_gyro, double dt)
{
  ...
  // atan2 在 ±π 处跳变时，用展开后的差分避免 d_phi0 / d_theta 虚假尖峰（见 unwrap）
  double delta_phi0 = phi0 - last_phi0;
  while (delta_phi0 > M_PI)
    delta_phi0 -= 2.0 * M_PI;
  while (delta_phi0 < -M_PI)
    delta_phi0 += 2.0 * M_PI;
  d_phi0 = delta_phi0 / dt;  // 计算phi0变化率，d_phi0用于计算lqr需要的d_theta
  d_alpha = 0.0 - d_phi0;
